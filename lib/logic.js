	...
	if (data.layout == 'post') {
        ...
        if (!abbrlink) {
            ...
            if (!/.*\.org/.test(data.source)){
				...
				var opt_AutoCategoryEnable = ((this.config.abbrlink && this.config.abbrlink.auto_category && this.config.abbrlink.auto_category.enable) ? this.config.abbrlink.auto_category.enable : true);
				var opt_AutoCategoryDepth = ((this.config.abbrlink && this.config.abbrlink.auto_category && this.config.abbrlink.auto_category.depth) ? this.config.abbrlink.auto_category.depth : 3);
				var overwrite = true;			
				if (opt_AutoCategoryEnable && overwrite) {					
					...
					if (categories.length-2 == 0 || depth == 0){						
						...
						if (!Array.isArray(tmpPost.categories) || (!tmpPost.categories.join("_") == newCategories.join("_"))) {
			...
			if (data.source.startsWith("_drafts/"))  {
				// is draft //
				log.i("length:",data.title.length);
				if(data.title.length==0) log.i("No title found for draft [source/_drafts/%s.md][ %s ]", data.slug, data.title);
				log.i("Generate link [%s] for draft [source/_drafts/%s.md][ %s ]", abbrlink, data.slug, data.title);
				//+++
				//From: hexo-auto-category
				//see:https://github.com/xu-song/hexo-auto-category
				//File: hexo-auto-category\lib\logic.js
				log.i("Generated: categories [%s] for draft [source/_drafts/%s.md][ %s ]", tmpPost.categories, data.slug, data.title);
				//+++
			} else {
				// is not draft //
				if(data.title.length==0) log.i("No title found for post [source/_posts/%s.md][ %s ]", data.slug, data.title);
				log.i("Generate link [%s] for post [source/_posts/%s.md][ %s ]", abbrlink, data.slug, data.title);
				//+++
				//From: hexo-auto-category
				//see:https://github.com/xu-song/hexo-auto-category
				//File: hexo-auto-category\lib\logic.js
				log.i("Generated: categories [%s] for post [source/_posts/%s.md][ %s ]", tmpPost.categories, data.slug, data.title);
				//+++
			}			
        }
        model.add(abbrlink);
